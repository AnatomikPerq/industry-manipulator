...
интерфейсе), `/api/sessions` (список СВОИХ сессий + состояние очередей, POST — создать) и
`/api/sessions/<id>/{rename,delete,upload,file-delete,set-type,set-llm,enqueue,
cancel,log,report,report.pdf,file,fragment}` и — для обычного чата (V2.1) —
...
пайплайна (`V2.1 beta`) от этого не меняется: сборка — это способ поставки, а не
