...
import { initLlmAdmin, openLlmAdmin } from "./llmadmin.js";
...
// ------------------------------------------------------------
async function init() {
  // Переключатель темы нужен и на экране входа — заводим до проверки токена.
  initTheme();
...
function renderUserArea() {
  // Кнопка «Настройки ИИ» в разделе сессий - только администратору.
  const adminBtnEl = $("btn-llm-admin");
  if (adminBtnEl) adminBtnEl.classList.toggle("hidden", !USER.is_admin);
...
// ------------------------------------------------------------
function bindEvents() {
  $("btn-llm-admin").addEventListener("click", openLlmAdmin);
  ...
  // Транскрипт обмена с ИИ за прогон - в соседней вкладке. Открываем нативно
  // (window.open), а не fetch'ем: cookie с токеном уйдёт сама, как у отчёта и
  // исходных документов.
  $("btn-llm-log").addEventListener("click", () => {
    if (S.id) window.open(`/api/sessions/${encodeURIComponent(S.id)}/lmstudio.log`, "_blank");
  });
  bindLLM();
