...
1. Одна и та же находка в обоих отчётах = ОДНА запись в итоге. Считай находки
   одинаковыми, если они указывают на одно и то же место и говорят об одном и том
   же по сути - даже если сформулированы разными словами. Одно и то же место -
   это совпадение document + клеммник/штифт, либо document + row, либо тот же
   KKS-тег, либо (для находок по связке документов одного шкафа) одно и то же
   ПОЗИЦИОННОЕ ОБОЗНАЧЕНИЕ (поле designator в refs) внутри одной связки.
...
6. Сохраняй оба рода находок: и междокументные (scope="cross_document"), и
   внутридокументные (scope="single_document"). У находки по связке документов
   (спецификация / сборочный чертёж / принципиальная схема) может быть ДО ТРЁХ
   ref'ов - НЕ выбрасывай третий, он показывает инженеру третий документ.
7. НЕ объединяй находки по РАЗНЫМ связкам (комплектам разных шкафов), даже если
   позиционные обозначения совпали: '1QF1' в одном шкафу и '1QF1' в другом - это
   разные аппараты. Связку смотри по имени документа в refs.
8. Отсортируй итоговый список по severity: critical -> high -> medium -> low -> info.
